...
@section('main')
    ...
    <div class="flex justify-center mt-10">
        <form class="bg-white shadow-md rounded border .border-customgreen px-8 pt-6 pb-8 mb-4 w-1/4 ml-10"
              action="{{ route('profile.updateaddress', ['address' => $address->id]) }}" method="POST">
            @method('PUT')
            @csrf
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline
                       ...
                       id="streetname" type="text" placeholder="Street name + house number" name="streetname" value="{{ old('streetname', $address->address) }}" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <select
                    ...
                    <option
                    @if( old('country', $address->country) == $address->country)
                        selected
                    @endif
                ...
                <select
                    ...
                    @foreach($addresstypes as $addresstype)
                        <option value="{{ $addresstype->id }}"
                                @if( old('addresstype', $address->addresstype_id) == $addresstype->id)
                                selected
                            @endif
